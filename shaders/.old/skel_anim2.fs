...
void main(){
    ...
    // FragColor = texture(material.texture_diffuse0, vs_out.TexCoords);
}

vec3 CalcDirLightBlinn(DirLight light, vec3 normal, vec3 viewDir){
    vec3 ambientVec3 = light.color * light.ambientStrength * 
        (vec3(texture(material.texture_diffuse0, vs_out.TexCoords)) + material.color);
    ...
    vec3 specularVec3 = specular * light.color * light.specularStrength * 
        (vec3(texture(material.texture_specular0, vs_out.TexCoords)) + material.color);
